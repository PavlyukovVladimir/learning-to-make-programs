...
<p>Задача будет состоять в том, что предварительно будет проведена индексация неких текстов, затем пользователь, вызывая ранжировку текста запускал например скрипт строку_запроса получал в ответ список названий текстов отранжированных по убыванию соответствия словам в строке запроса выглядеть должно как-то так:</p>
</p>Где и как хранятся тексты в задаче не сказано, логично сделать некую функцию indexer(files), которая индексирует тексты, а
files это отсортированный по названиям файлов список кортежей (название файла, путь к папке с файлом текста) скрипт indexer.py создаст список files, у всех кортежей будет один и тотже путь к папке с файлом текста, но зато если условия хранения текстов станут другими нужно будет изменить только ту часть скрипта, которая создает список files</p>
<p></p>
<p></p>
<p></p>
